...
public final class Triangles {
    ...
    double getRadian( Vector A, Vector B, Vector C ) {
        ...
        double res = Vectors.dot( AB, AC ) / ( AB.norm() * AC.norm() );
        // check to avoid NaN
        assert MyMath.doubleCompare( res, 1 ) <= 0 : res + " " + Vectors.dot( AB, AC ) + " " + AB.norm() * AC.norm();
    ...
    boolean toLeft( Vector point1, Vector point2, Vector point3 ) {
        double res = areaTwo( point1, point2, point3 );
        return res >= 0 || MyMath.isEqualFloats( res, 0 );
    }
    ...
    public static
    double areaTwoByLTL( Vector point1, Vector point2, Vector point3 ) {
    ...
    int sortByPolar( Vector point1, Vector point2 ) {
        ...
        if ( res == 0 ) {
            double area = areaTwo( Vector.origin, point1, point2 );
            if ( area == 0 ) return 0;
    ...
    int sortByToLeft( Vector point1, Vector point2 ) {
        double res = MyMath.quadrant( point1.xRelativeToLTL, point1.yRelativeToLTL ) -
                MyMath.quadrant( point2.xRelativeToLTL, point2.yRelativeToLTL );
        if ( res == 0 ) {
            double area = areaTwoByLTL( Vector.origin, point1, point2 );
            if ( area == 0 ) return 0;
            else if ( area > 0 ) return -1;
            return 1;
        }
