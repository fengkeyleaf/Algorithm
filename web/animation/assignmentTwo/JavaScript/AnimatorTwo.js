...
export default class AnimatorTwo extends Animator {
    ...
    constructor() {
        ...
        // this.e = 0.2; // 0 <= e <= 1
        this.e = 0.8; // 0 <= e <= 1
        this.impulseStriking = new THREE.Vector3( -400, 0, 5 ); // kg * m / s ^ 2
    ...
    #initCushionsDynamics() {
        let mass = 10; // kg
        // cushions
        this.cushion = new Cushion( this.meshPlane, mass, Dynamics.yAxis, "cushion" );
    ...
    static renderTwo() {
        ...
        if ( current - animator.startingDate >= 50 ) {
        ...
        if ( animator.isInit ) {
            objects[ 0 ].fs.add( animator.impulseStriking.clone() );
            // objects[ 1 ].fs.add( animator.impulseStriking );
            tempDt = dt;
            dt = 1;
        ...
        // check to see if balls are falling into pockets
        Pocket.isRemoved( animator.balls, animator.pockets, animator.outOfTable );
    ...
    #initCushions() {
        ...
        this.meshPlane.translateY( -this.#radius );
    ...
    #initPockets() {
        ...
        const geometry = new THREE.CircleGeometry( radius, segments, thetaStart );
        const leftTopPocketMesh = new THREE.Mesh( geometry, material )
        leftTopPocketMesh.position.set( -x, -this.#radius + offset, -z );
        leftTopPocketMesh.rotateOnAxis( Dynamics.xAxis, this.#rightAngleInRadian );
        this.scene.add( leftTopPocketMesh );

        const leftBottomPocketMesh = new THREE.Mesh( geometry, material )
        leftBottomPocketMesh.position.set( -x, -this.#radius + offset, z );
        leftBottomPocketMesh.rotateOnAxis( Dynamics.xAxis, this.#rightAngleInRadian );
        this.scene.add( leftBottomPocketMesh );

        const rightTopPocketMesh = new THREE.Mesh( geometry, material )
        rightTopPocketMesh.position.set( x, -this.#radius + offset, -z );
        rightTopPocketMesh.rotateOnAxis( Dynamics.xAxis, this.#rightAngleInRadian );
        this.scene.add( rightTopPocketMesh );

        const rightBottomPocketMesh = new THREE.Mesh( geometry, material )
        rightBottomPocketMesh.position.set( x, -this.#radius + offset, z );
        rightBottomPocketMesh.rotateOnAxis( Dynamics.xAxis, this.#rightAngleInRadian );
        this.scene.add( rightBottomPocketMesh );

        this.pockets = [];
        this.leftTopPocket = new Pocket( leftTopPocketMesh, "lefTop", radius );
        this.leftBottomPocket = new Pocket( leftBottomPocketMesh, "leftBottom", radius );
        this.rightTopPocket = new Pocket( rightTopPocketMesh, "rightTop", radius );
        this.rightBottomPocket = new Pocket( rightBottomPocketMesh, "rightBottom", radius );
        this.pockets.push( this.leftTopPocket );
        this.pockets.push( this.leftBottomPocket );
        this.pockets.push( this.rightTopPocket );
        this.pockets.push( this.rightBottomPocket );
    }
