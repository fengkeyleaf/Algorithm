...
public class EventSite extends Event {
    ...
    // <pj, pi, pk>
    public final List<VorRBTNode> triple = new ArrayList<>( 3 );
    // <pj, pi>, <pj, pk>.
    ...
    public VoronoiFace leftArc;
    public VoronoiFace rightArc;
    ...
    public VorRBTNode circleEvent;
    public VorRBTNode leftCircleEvent;
    public VorRBTNode rightCircleEvent;

    /**
     * Constructs to create an instance of EventEdge, status ( internal node )
     */

    public EventSite( double x, double y,
    ...
    public EventSite( Vector point, VoronoiFace left,
    ...
    public EventSite( VoronoiFace site, Type type ) {
    ...
    public EventSite( Circle circle, Vector lowest ) {
    ...
    public void addInternals( VorRBTNode left, VorRBTNode right ) {
    ...
    public VorRBTNode update( VorRBTNode node, StatusRBTreeVoronoi statusTree ) {
    ...
    public void updateTriple( StatusRBTreeVoronoi statusTree ) {
    ...
    public void updateNode( VorRBTNode node ) {
        if ( type == Type.CIRCLE )
    ...
    // this one is left
    public void updateCircleEvent( EventSite right ) {
    ...
    public boolean isOnInitSweep( double initY ) {
        if ( type == Type.SITE )
    ...
    public boolean isOnSweepLine( double sweepY ) {
    ...
    // TODO: 2/11/2022 change this method avoiding to mix x and y.
    @Override
    public void updateY( double x ) {
        Line directrix = new Line( new Vector( 1, x ), new Vector( 2, x ) );
        ...
        assert intersections != null : leftArc + " " + rightArc + " " + x;
        ...
        if ( leftArc.site.isBelow( rightArc.site ) ) {
            assert rightArc.site.isRight( intersections[ 1 ] ) : this + " " + x + " " + Arrays.toString( intersections );
            setXAndY( intersections[ 0 ] );
            return;
        }

        assert rightArc.site.isBelow( leftArc.site ) : rightArc.site + " " + leftArc.site;
        assert leftArc.site.isRight( intersections[ 1 ] ) : this + " " + x + " " + leftArc.site + " " + Arrays.toString( intersections );
    ...
    public void getDirection( double sweepY ) {
        EventSite endPoint = new EventSite( this );
        endPoint.updateY( sweepY );
    ...
    public boolean isNotSpecial( EventSite e ) {
    ...
    boolean hasCircleEvent( EventSite current, EventSite other, double sweepY ) {
        ...
        internalOffset.updateY( OFFSET_Y );
        leafOffset.updateY( OFFSET_Y );

        // get the intersection of the two voronoi edges
        Vector intersection = current.bisector.lineIntersect( other.bisector );
