...
public class StatusRBTreeVoronoi
    ...
    public StatusRBTreeVoronoi( Comparator<EventSite> c ) {
        super( c );
    }

    public StatusRBTreeVoronoi() {
        this( null );
    }
    ...
    public List<EventSite> getDanglingEdges( double sweepY ) {
    ...
    protected int compareKeys( VorRBTNode root, EventSite key ) {
        ...
        if ( !root.key.isOnSweepLine( sweepY ) ) root.val.updateY( sweepY );
        if ( !key.isOnSweepLine( sweepY ) ) key.updateY( sweepY );
        return CompareElement.compare( comparator, root.key, key );
    }
