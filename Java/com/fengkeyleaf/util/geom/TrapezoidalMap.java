...
public final class TrapezoidalMap {
    ...
    public static
    void drawTrapezoidalMap( SearchStructure SS,
                             DrawingProgram drawer, List<Vector> points ) {
    ...
    static
    void drawTrapezoidalMap( DrawingProgram drawer, List<Face> faces,
                             List<Vector> points, Color poly, Color point ) {
        faces.forEach( f -> drawer.drawPoly( poly, f ) );
        drawer.drawPoints( point, points );
    }

    public static
    void drawDs( List<SearchVertex> Ds, DrawingProgram drawer ) {
        List<Face> faces = new ArrayList<>();
        Ds.forEach( vertex -> {
            faces.add( vertex.trapezoid.getDCEL());
        } );

        faces.forEach( f -> drawer.drawPoly( DrawingProgram.INTERSECTION_COLOR, f ) );
    }

    public static
    void drawQuery( SearchVertex d, Vector query,
        ...
        switch ( d.type ) {
            ...
            case SEGMENT -> {
                lines.add( d.line );
                drawer.drawLines( DrawingProgram.INTERSECTION_COLOR, lines );
            }
