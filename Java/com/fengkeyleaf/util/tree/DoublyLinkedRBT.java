...
public class DoublyLinkedRBT<K, V> extends RedBlackTree<K, V>
     ...
     * in this doubly-linked RBT in sequential order.
     * */
    ...
    /**
     * Returns an iterator over the elements
     * in this doubly-linked RBT in reverse sequential order.
     * */

    public Iterator<MapTreeNode<K, V>> descendingIterator() {
    ...
    protected boolean check() {
        ...
        boolean isRightSize = linkedList.size() == super.size();

        if ( !isDoublyConnected )       System.err.println( "Not doublyConnected" );
        if ( !isLinked )       System.err.println( "Not Linked" );
        if ( !isRightSize ) System.err.println( "Not right size" );

        return isBBST && isDoublyConnected && isLinked && isRightSize;
    }
}
