 ...
 *
 * Application: Related POJ problem:
 * @see <a href="http://poj.org/problem?id=2991">Crane</a>
 *
 * And note that segment tree can also be applied to
 * Windowing Query in computational geometry.
 * But the segment tree here don't have the ability to do so.
 * Look into {@link com.fengkeyleaf.util.geom.SegmentTree},
 * if you wanna know how it works in the context of computational geometry.
 *
 * Reference resource:
...
public class SegmentTree<E> extends PerfectBinaryTree<SegmentTree<E>.SegmentNode> {
    ...
    class SegmentNode extends Node {
        ...
        SegmentNode( E threshold ) {
            super( IDStatic++ );
            this.max = this.min = threshold;
        }

        SegmentNode( E minThreshold,
                     E maxThreshold ) {
            super( IDStatic++ );
        ...
        SegmentNode( int ID, E maxThreshold,
                     E minThreshold ) {
            super( ID );
            this.max = maxThreshold;
            this.min = minThreshold;
    ...
    private void initialize( List<E> elements, int index, int left, int right ) {
        ...
        if ( left >= elements.size() ) {
            tree.set( index, new SegmentNode( null ) );
            return;
        }
        // one element left
        if ( left + 1 >= right ) {
            assert left + 1 == right;
            tree.set( index, new SegmentNode( elements.get( left ) ) );
            return;
        }
    ...
    private void update( SegmentNode node,
    ...
    public void update( int index, E element ) {
        ...
        SegmentNode node = tree.get( index );
        node.min = element;
        node.max = element;
        ...
        while ( index > 0 ) {
            ...
            node = tree.get( index );
            SegmentNode left = tree.get( getChildrenIndex( index, true ) );
            SegmentNode right = tree.get( getChildrenIndex( index, false ) );
