...
public class Line implements UpdateCoordinatesShape {
    ...
    private double updateY( double x ) {
        assert B != 0 : this;
        return ( C - x * A ) / B;
    }
    ...
    public void updateYAndX( Vector target, Vector update, boolean isUpdatingByX ) {
        ...
        if ( isVertical ) {
            ...
            if ( MyMath.isEqualFloats( update.x, verticalX ) ) {
                // yes, update Y
        ...
        // Horizontal line
            ...
            if ( MyMath.isEqualFloats( update.y, horizontalY ) ) {
                // yes, update x
    ...
    public boolean isOnThisLine( Vector vector ) {
        if ( isVertical ) return MyMath.isEqualFloats( vector.x, verticalX );
        else if ( isHorizontal ) return MyMath.isEqualFloats( vector.y, horizontalY );

        return MyMath.isEqualFloats( updateY( vector.x ), vector.y );
    }
    ...
    private boolean isVertical() {
        return MyMath.isEqualFloats( startPoint.x - endPoint.x, 0 );
    }

    private boolean isHorizontal() {
        return MyMath.isEqualFloats( startPoint.y - endPoint.y, 0 );
    }
...
//        System.out.println( GeometricIntersection.linesIntersect( line3, line1 ) );
        ...
        System.out.println( MyMath.isEqualFloats( line4.updateY( x ), line3.updateY( x ) ));
    }
